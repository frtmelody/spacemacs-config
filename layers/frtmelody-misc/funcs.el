...
(defun frtmelody/get-chrome-tabs ()
  "返回 Chrome 标签."
  (let ((script
         (mapconcat
          #'identity
...
(defun frtmelody/chrome-switch-tab-internal (window-id tab-id)
...
(defun frtmelody/chrome-switch-tab (window-id tab-id)
  (interactive
   (let* ((tabs (frtmelody/get-chrome-tabs))
          (input
           (completing-read
            "Open Chrome Tab: "
            (mapcar
